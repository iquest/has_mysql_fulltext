module HasMysqlFulltext
  module Index
    class Base < ActiveRecord::Base
      ...
      scope :attributes_match, lambda { |attr, expr| where(:indexable_attribute => attr).where("MATCH(fulltext_indices.data) AGAINST (? IN BOOLEAN MODE)",  prepare_expression(expr.strip)) }
