module HasMysqlFulltext
  module Index
    class Base < ActiveRecord::Base    
      ...
      scope :match, lambda { |expr| where("MATCH(fulltext_indices.data) AGAINST (? IN BOOLEAN MODE)",  expr.strip) }
      scope :attribute_match, lambda { |attr, expr| where("indexable_attribute = ?", attr).where("MATCH(fulltext_indices.data) AGAINST (? IN BOOLEAN MODE)",  expr.strip) }
    end
  end
end